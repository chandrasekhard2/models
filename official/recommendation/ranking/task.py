...
def _get_tpu_embedding_feature_config(
  ...
  for i, vocab_size in enumerate(vocab_sizes):
    ...
    #tf._logging.info(f'vocab_size: {vocab_size}')
    #tf._logging.info(f'embedding_dim: {embedding_dim[i]}')
    #tf._logging.info(f'bound: {bound}')
...
class RankingTask(base_task.Task):
  ...
  def build_model(self) -> tf.keras.Model:
    ...
    embedding_optimizer = tf.keras.optimizers.legacy.Adagrad(
        learning_rate=self._optimizer_config.lr_config.learning_rate,
        initial_accumulator_value=self._optimizer_config.initial_accumulator_value,
        epsilon=self._optimizer_config.epsilon
    )
